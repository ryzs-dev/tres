...
export const handleContentfulHookWorkflow = createWorkflow(
  ...
  (input: HandleContentfulHookWorkflowInput) => {
    ...
    when(
      input,
      (input) => input.entry.sys.contentType.sys.id === "product"
    ).then(() => {
    ...
    when(
      input,
      (input) => input.entry.sys.contentType.sys.id === "productVariant"
    ).then(() => {
    ...
    when(
      input,
      (input) => input.entry.sys.contentType.sys.id === "productOption"
    ).then(() => {
      updateProductOptionsWorkflow.runAsStep({
        input:
          prepareUpdateData as unknown as UpdateProductOptionsWorkflowInput,
      });
    });
  }
);
