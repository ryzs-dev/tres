...
export const prepareBundleCartDataStep = createStep(
  ...
  async ({ bundle, quantity, items }: PrepareBundleCartDataStepInput) => {
    const bundleItems = bundle.items.map((item: BundleItemWithProduct) => {
      ...
      // Skip if optional and not selected
      if (!selectedItem) {
        // ✅ Skip optional items without selection
        if (!item.optional) {
          throw new MedusaError(
            MedusaError.Types.INVALID_DATA,
            `No variant selected for required bundle item ${item.id}`
          );
        } else {
    ...
    return new StepResponse(
      bundleItems.filter((item) => item !== null) // ✅ Filter nulls
    );
  }
);
