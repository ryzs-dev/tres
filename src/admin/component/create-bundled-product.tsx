...
const CreateBundledProduct = () => {
  ...
  const handleCreate = async () => {
    try {
      ...
      setTitle("");
      setItems([{ product_id: undefined, quantity: 1, optional: false }]);
    } catch (error) {
      ...
      <FocusModal.Content>
        ...
        <FocusModal.Body>
          <div className="flex flex-1 flex-col items-center overflow-y-auto">
            <div className="mx-auto w-full max-w-[720px] flex-col gap-y-8 px-2 py-16">
              <div>
        ...
        <Select.Content>
          {products?.map((product, i) => (
            <Select.Item
              key={product.id}
              value={product.id}
              ref={i === products.length - 1 ? lastOptionRef : null}
            >
              {product.title}
            </Select.Item>
          ))}
        ...
        <div className="flex items-center gap-2 mt-6">
          <input
            ...
            onChange={(e) =>
              setItems((items) =>
                items.map((i, idx) =>
                  idx === index ? { ...i, optional: e.target.checked } : i
                )
              )
            }
          />
